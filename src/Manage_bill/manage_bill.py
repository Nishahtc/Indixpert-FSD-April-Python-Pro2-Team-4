...
class ManageBill(Bill):
    ...
    def create_bill(self, bill_id, customer_name, customer_phone_no, items, quantity, table_number, price, date_time ):
        bill_id = str(uuid.uuid4())[:6]
        date_time = datetime.now()
        new_bill = BillModel(self, bill_id, customer_name, customer_phone_no, items, quantity, table_number, price, date_time)
        self.bills.append(new_bill)
        self.save_bill()
        print("Bill Created successfully")

    def update_bill(self, bill_id, items, quantity, price):
    ...
    def get_bill(self, bill_id):
        for bill in self.bills:
            if(bill.bill_id == bill_id ):
                ...
                print(f'  -'*135)
                print(f"{bill.bill_id:<10}  {bill.customer_phone_no :<15} {bill.customer_name:<20} {bill.table_number:<15} {bill.items:<30} {bill.quantity:<10} {bill.total_amount:<15} {bill.date_time.strftime('%Y-%m-%d %H:%M:%S'):<20}")
                break
